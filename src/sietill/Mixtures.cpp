...
void MixtureModel::print_coding_progress() {
  std::cout << "MixtureModel coding progress: \n"
            << "Currently working modes: \n"
            << "Sum approach: mixture pooling    \n"
            << "Max approach: no pooling, mixture & global pooling \n"
            << std::endl;
}
...
void MixtureModel::reset_accumulators() {
  // TODO: implement
	...
	std::fill(var_accumulators_.begin(), var_accumulators_.end(), minimal_variance_value_); // Avoid numerical instability
...
void MixtureModel::finalize() {
  ...
  if (var_model == GLOBAL_POOLING) {
    // we collect the means of the whole dataset and use it to calculate the variance afterwards
    std::vector<double> global_mean  = std::vector<double>(dimension, 0.0);
    for (StateIdx m = 0; m < mixtures_.size(); m++) {
      for (DensityIdx d = 0; d < mixtures_[m].size(); d++) {
        std::transform(global_mean.begin(), global_mean.end(),
                       mean_accumulators_.begin() + mixtures_[m][d].mean_idx * dimension,
                       global_mean.begin(),
                       std::plus<double>());
      }
    ...
    std::transform(global_mean.begin(), global_mean.end(),
                   global_mean.begin(), std::bind2nd(std::divides<double>(), total_observations));

    // calculate the variance with the updated means
    DensityIdx var_idx  = 0;
    calculate_variance(var_idx, global_mean.begin());
  }

  if (verbosity_ > noLog) {
  	std::cout << "Accumulation output: " << std::endl;
...
void MixtureModel::update_split_densities(MixtureDensity& md_original, MixtureDensity& md_split) {
  ...
  mean_weight_accumulators_[mean_idx_split] = mean_weight_accumulators_[mean_idx_original];

  /*
  std::copy(mean_accumulators_.begin() + mean_idx_original * dimension,
  		      mean_accumulators_.begin() + mean_idx_original * dimension + dimension,
  		      mean_accumulators_.begin() + mean_idx_split    * dimension );
	*/
