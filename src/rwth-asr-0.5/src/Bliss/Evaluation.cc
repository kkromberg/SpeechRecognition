...
// $Id: Evaluation.cc 7257 2009-07-10 12:17:00Z rybach $
    ...
    graphDumpChannel_(config, "dump-graphs")
{
	...
	lemmaToLetter_ = lexicon_->createLemmaToOrthographyTransducer(false);
    }
...
void Evaluator::setReferenceTranscription(const std::string &referenceTranscription) {
	...
	if (Fsa::isEmpty(correct_.phon)) {
	    error("The set of correct phoneme sequences is empty.  Phoneme errors cannot be computed.");
	    correct_.phon.reset();
	}
    } else {
	correct_.phon.reset();
