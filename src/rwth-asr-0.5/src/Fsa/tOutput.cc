...
namespace Ftl {
	...
	*/
	...
	Fsa::ConstAlphabetRef input_, output_;
    public:
	WriteLinearDfsState(_ConstAutomatonRef f, std::ostream &o) :
	    Precursor(f), first_(true), o_(o), input_(Precursor::fsa_->getInputAlphabet()), output_(Precursor::fsa_->getOutputAlphabet()) {}
	void discoverState(_ConstStateRef sp) {
	    for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
		std::string out;
		if ((a->input() != Fsa::Epsilon) && (a->input() <= Fsa::LastLabelId))
		    out = input_ ? input_->symbol(a->input()) : Core::itoa(a->input());
		if ((Precursor::fsa_->type() == Fsa::TypeTransducer) && (a->output() != Fsa::Epsilon))
	...
	if (o) {
	    Core::ProgressIndicator *p = 0;
	    if (progress) p = new Core::ProgressIndicator("writing", "states");
	    WriteLinearDfsState<_Automaton> s(f, o);
	    s.dfs(p);
	    delete p;
	}
	o << std::endl;
