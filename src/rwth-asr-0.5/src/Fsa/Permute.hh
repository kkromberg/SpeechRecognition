...
namespace Fsa {
	...
	virtual void permuteArcs(State *sp) const {
		...
		if (!Precursor::states_[s].used_[i] && (size_t(abs(i - Precursor::states_[s].depth_)) < Precursor::distortionLimit_)) { // !!! LIMIT
		    ...
		    Arc *a = sp->newArc();
		    *a = Precursor::arcs_[i];
		    a->target_ = insertState(depth, Precursor::states_[s].used_, i);
	...
	virtual void permuteArcs(State *sp) const {
		...
		if (!Precursor::states_[s].used_[i]) {
		    Arc *a = sp->newArc();
		    *a = Precursor::arcs_[i];
		    a->target_ = insertState(Precursor::states_[s].depth_+1, Precursor::states_[s].used_, i);
	...
	virtual void permuteArcs(State *sp) const {
		...
		*a = Precursor::arcs_[firstBitNotSet];
		a->target_ = insertState(Precursor::states_[s].depth_+1, Precursor::states_[s].used_, firstBitNotSet);
