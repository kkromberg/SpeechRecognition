...
namespace Ftl {
	...
	class SsspBackward4SpecialSymbols : public DfsState<_Automaton> {
		...
		SsspBackward4SpecialSymbols(Fsa::ConstAutomatonRef f) :
			...
			{
			require(f->hasProperty(Fsa::PropertyAcyclic));
			this->dfs();
		}
	    ...
	    _Weight totalFlow() const { return potential_[this->fsa_->initialStateId()]; }
		...
		struct AddArc {
			...
			inline static _Weight failArc(const _Arc *arc, DirectHits &directHits, _Accumulator *collector, const bool firstLevel, _SsspBackward4SpecialSymbols *owner){
				Fsa::StateId ts = arc->target();
				_Weight failWeight;
				failWeight=owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,_Weight>(arc->weight()), owner->processState<AddArc>(ts, directHits));
				collector->feed(failWeight);
				return failWeight;
			}
		};
		...
		struct AddArcTropical : AddArc {
			inline static _Weight failArc(const _Arc *arc, DirectHits &directHits, _Accumulator *collector, const bool firstLevel, _SsspBackward4SpecialSymbols *owner){
				Fsa::StateId ts = arc->target();
				_Weight failWeight;
				failWeight=owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,_Weight>(arc->weight()), owner->potential_[ts]);
				_Weight currentGet = collector->get();
				if (owner->semiring_->collect(currentGet, failWeight) != currentGet)
					failWeight=owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,_Weight>(arc->weight()), owner->processState<AddArcTropical>(ts, directHits));
				collector->feed(failWeight);
		...
		struct AddArcNegCollect : AddArc {
			...
			inline static _Weight failArc(const _Arc *arc, DirectHits &directHits, _Accumulator *collector, const bool firstLevel, _SsspBackward4SpecialSymbols *owner){
				...
				if (firstLevel) {
					...
					failWeight=owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,_Weight>(arc->weight()), failWeight);
					collector->feed(failWeight);
				} else {
					failWeight=owner->semiring_->zero();
				}
				return failWeight;
			}
		};
	...
	class SsspForward4SpecialSymbols {
		...
		struct AddArc {
			inline static void regularArcWithoutDirectHits(const _Arc *arc, const _Weight &sourceForwardPotential, ForwardPotential &fpot, _SsspForward4SpecialSymbols *owner) {
				Fsa::StateId ts = arc->target();
				owner->getForwardPotential(ts).feedForwardPotential(owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,_Weight>(arc->weight()), sourceForwardPotential));
			}
		...
		void finishStates(const _Stack states) {
			...
			for (_Stack::const_iterator s=states.begin(); s!=states.end(); ++s) {
				...
				if (state->isFinal()) {
					...
					//std::cout << " before fwdFlowAcc->get()=" << (f32)fwdFlowAcc->get() << " feed(" << (f32)semiring_->extend(fStatePot, state->weight()) << ")" << std::endl;
	...
	class BestAutomaton4SpecialSymbols : public SlaveAutomaton<_Automaton> {
		...
		bool getBestArc(typename _State::const_iterator &bestArc, Fsa::StateId s, DirectHits &directHits) const {
			...
			for (typename _State::const_iterator a = _sp->begin(); a != _sp->end(); ++a) {
				...
				if (a->output() == Fsa::Failure or a->input() == Fsa::Failure) {
					...
					if (Precursor::fsa_->semiring()->compare(w, minWeight) < 0) {
						minWeight = w;
						return getBestArc(bestArc, ts, directHits);
					}
				} else {
					noNoneFailArc=false;
		...
		*/
		virtual _ConstStateRef getState(Fsa::StateId s) const {
			...
			DirectHits directHits;
			if (getBestArc(bestArc, s, directHits)) {
				sp->unsetFinal();
				*sp->newArc() = *bestArc;
			}
