...
namespace Signal {
	...
	void printClustering()
	{
		...
		for (u32 i = 0; i<nrsegments_; i++) {
			...
			Core::XmlAttribute("value", classid_[i]);
		}
	...
	{
	    cluster(modelEstimator_.finalize()); /// @todo: ugly, this should be moved to the work method, but how?
	}

	virtual bool setParameter(const std::string &name, const std::string &value)
	{
	    if (paramMincluster.match(name))
