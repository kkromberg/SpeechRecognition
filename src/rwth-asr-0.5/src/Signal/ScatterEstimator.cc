...
void ScatterMatrixEstimator::accumulate(const Math::Vector<Data> &x)
{
    require_(!needInit_);
...
void ScatterMatricesEstimator::accumulate(ClassIndex classIndex, const Math::Vector<Data> &x)
{
    require_(classIndex < nClasses_);
    if (needInit_) initialize();
    vectorSums_[classIndex] += x;
    ++ counts_[classIndex];
    Precursor::accumulate(x);
}
