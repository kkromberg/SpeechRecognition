 ...
 *  Override the following functions to implement a concrete filter:
...
public:
	...
	Frequency center, Frequency width, Frequency maximumFrequency,
...
bool FilterBank::FilterBuilder::setStart()
{
	...
	std::max(center_ - normalizedCenterPosition() * width_, 0.0));
...
public:
    ...
    virtual Frequency center(size_t filterIndex) const {
	return spacing_ * filterIndex + normalizedCenterPosition_ * filterWidth_;
    }
    virtual size_t getNumberOfFilters() const {
	return (size_t)Core::floor(postprocessNumberOfFilters(
				       (maximumFrequency_ - filterWidth_) / spacing_ + 1));
    }
    ...
    Math::UnaryAnalyticFunctionRef warpingFunction)
{
    ...
    Precursor::init(filterWidth, spacing, normalizedCenterPosition,
		    maximumFrequency, warpingFunction);

    size_t nFilters = getNumberOfFilters();
    Frequency coverage = (spacing_ * (nFilters - 1) + filterWidth_) / maximumFrequency_;
...
bool FilterBank::init()
{
	...
	!boundary_->init(filterWidth_, spacing_,
			 builder_->normalizedCenterPosition(), maximumFrequency_,
	...
	filters_[i] = builder_->create(boundary_->center(i),
				       boundary_->filterWidth(), maximumFrequency_,
...
const Core::ParameterFloat FilterBankNode::paramFilteringInterval(
    "filtering-interval", "Filters are placed only inside this interval.", Core::Type<f32>::max, 0);
