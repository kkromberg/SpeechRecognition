...
namespace Signal {
     ...
     *  -Filters are placed in a constant distant given by the spacing.
     *   @param shouldWarpCenterPositions_ controls over which axis the spacing is constant.
     *    -yes:filters are placed in a constant distant over the warped axis.
     *    -no:filters are placed in a constant distant over the original unwarped axis.
     *        Implementation is done by simply pre-warping the center position, since the inverse warping
     *        applied which building the filters will yield equidistant filters over the original unwarped axis.
     *        Note that the filter widths are warped in this cases.
     *  -If spacing is set to zero, it will be set automatically to normalized-center * filter-width,
     *   i.e. the size of the left flank.
     *  -Parts of filters reaching out of the interval [0..maximal-frequency] are assumed to be zero.
     *  -Position of first and last filters are controlled by different boundary type classes,
     ...
     *                       parameter resolving with values arriving one or more ports. Example:
     *                       nest(linear-2($input(warping-factor), 0.875), mel). In this example the node
     *                       creates an input port called warping-factor and the values arriving will be
