...
// $Id: StateTree.cc 8249 2011-05-06 11:57:02Z rybach $
...
u32 StateTree::hashSequence(const PronunciationSuffix &s) const {
	u32 result = u32(s.pronunciation());
...
public:
	...
	StateDesc head() const {
		require(hasUniqueHead());
		return model_->head(items_.front());
	}

private:
	void add(const PronunciationSuffix &s) {
		require_(!hasUniqueHead() || items_.empty() || head() == model_->head(s));
		items_.push_back(s);
	}
...
public:
	...
	void addSorted(const PronunciationSuffix &s) {
		require_(items_.empty() || !PronunciationSuffix::Ordering(model_)(s, items_.back()));
		add(s);
		updateHash(s);
	}
...
void StateTree::SuffixSet::partition(SuffixSetList &conts) const {
	...
	for (ItemList::const_iterator i = items_.begin(); i < items_.end(); ++i) {
		verify(!i->isEmpty());
		const StateDesc head(model_->head(*i));
