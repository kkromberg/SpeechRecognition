    ...
    Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment *s)
{
    ...
    // calculate the posterior automaton with expectation semiring of Q(Z), with Z = (A,P*A)
    // A = accuracies, P = scaled joint probabilities
    Lattice::ConstWordLatticeRef denominator = Lattice::getPart(lattice, part_);
    Fsa::Weight expectation;
    Fsa::ConstAutomatonRef fsa =
    Fsa::posteriorE(
	    Fsa::changeSemiring(denominator->mainPart(), Fsa::LogSemiring),
