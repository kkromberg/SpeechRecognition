	...
	noDependencyCheck_(paramNoDependencyCheck(c))
{
	...
	latticeArchiveWriter_ = Lattice::Archive::openForWriting(select("lattice-archive"), lexicon_);
...
void OfflineRecognizer::processResult(Bliss::SpeechSegment *s) {
    ...
    Lattice::ConstWordLatticeRef lattice = recognizer_->getCurrentWordLattice();
    if (lattice) {
	if (timeConditionedLattice_) {
	    lattice = Lattice::timeConditionedWordLattice(lattice);
	}
	if (latticeArchiveWriter_) {
	    latticeArchiveWriter_->store(s->fullName(), lattice);
	}
