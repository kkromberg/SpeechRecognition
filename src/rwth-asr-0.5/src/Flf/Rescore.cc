...
namespace Flf {
	...
	virtual std::string describe() const {
	    return Core::form("%s(%s,dim=%d,const=%f)", Func::describe().c_str(), fsa_->describe().c_str(), id_, f_.c);
	}
    };
    template<class Func>
	...
	virtual std::string describe() const {
	    return Core::form("extendByPenalty(%s,dim=%d,penalty=%f)", fsa_->describe().c_str(), id_, penalty_);
	}
    };
	...
	virtual std::string describe() const {
	    return Core::form("extendByPronunciation(%s,dim=%d)", fsa_->describe().c_str(), id_);
	}
    };
