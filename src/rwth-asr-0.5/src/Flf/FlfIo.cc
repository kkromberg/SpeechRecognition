...
namespace Flf {
	...
	if (desc->scoreFiles.size() != semiring_->size())
	    Core::Application::us()->criticalError(
		"FlfContext: Semiring dimension mismatch, expected %d, got %d",
    ...
    */
	...
	private:
		...
		if (n_ == 0) {
		    ...
		    parser()->error("id %d found, but dimensionality is set to %d", id_, (u32)n_);
	    }
		...
		if (!Core::strconv(cdata_, desc_->scales[id_]))
		    parser()->error("could not parse scale[%d]: %s", id_, cdata_.c_str());
		cdata_.clear();
	    }
