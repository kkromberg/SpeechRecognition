...
const Allophone * AllophoneAlphabet::allophone(Fsa::LabelId id) const {
    require(size_t(id & IdMask) < allophones_.size());
    return allophones_[id & IdMask];
}

Fsa::LabelId AllophoneAlphabet::index(const Allophone *allo) const {
...
Fsa::LabelId AllophoneStateAlphabet::index(const Allophone *allo, s16 state) const {
    verify(allo && (state <= MaxStateId));
    return allophoneAlphabet_->index(*allo) | state << 26;
}
...
Fsa::LabelId AllophoneStateAlphabet::index(const std::string &symbol) const {
    //
