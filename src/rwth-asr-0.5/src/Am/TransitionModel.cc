 ...
 * that are repreated or skipped are on the input side of the
 ...
 * maintainability of the code, over the small increase in efficieny.)
 ...
 * implement.  (In "discharded" states they are not allowed.)
 ...
 * arcs needed.  So discharging is the preferrable alternative.  The
 ...
 * target state.  In "favourable" situations this is optimized into a
 * single transitions (skip optimization).  If you have read so far,
 * you are certainly able to figure out what these favourable
 * conditions are.
 *
    ...
    stateModel_(stateModel)
{
	...
	const Bliss::Phoneme *phone = pi->phoneme(*s);
	AllophoneIndex ai = allophones.index(
	    Allophone(phone->id(), Allophone::isInitialPhone | Allophone::isFinalPhone));
	const Allophone *allophone = allophones.allophone(ai);
