...
private:
	...
	struct Hash
	{
	    ...
	    {
		return size_t(pbssd.phoneBoundaryFlag) << 1 + size_t(
			pbssd.isCoarticulated);
	    }
	};
    };
...
{
    bool coartSilence = false; // not used yet, but enables the support for cooarticulated silence
