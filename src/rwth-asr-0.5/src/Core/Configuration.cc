...
// $Id: Configuration.cc 7257 2009-07-10 12:17:00Z rybach $
...
/*****************************************************************************/
{
	...
	if (parseInclude(line, includeFilename, warningDescription)) {
	    ...
	    if (!isRegularFile(f))
		f = joinPaths(directoryName(filename), includeFilename);
	    if (isRegularFile(f))
		setFromFile(f, selection);
	    else
		error(filename, linenr, "Include file '" + includeFilename + "' does not exist.");
