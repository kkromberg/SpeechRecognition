...
// $Id: Statistics.cc 8249 2011-05-06 11:57:02Z rybach $
...
Core::HistogramStatistics::HistogramStatistics(std::string name, u32 bins) :
    Precursor(name),
    scale_(1),
    bins_(bins) {
}
...
void Core::HistogramStatistics::operator+=(unsigned int v) {
    ...
    if(counts_.size() <= binOffset)
	counts_.resize(binOffset ? 2*binOffset : 1, 0);

    counts_[binOffset] += 1;

    //We keep the array significantly larger than the count of bins, so we have some more precision
    while(counts_.size() > bins_ * 1000) {
	//Scale, to save some memory. Halve the size of the table.
	u32 newScale = scale_ * 2;
	std::vector<u32> newCounts(counts_.size() / 2, 0);

	for(u32 a = 0; a < counts_.size(); ++a)
	    newCounts[a/2] += counts_[a];
