...
#include <sys/param.h>
struct BackingOffLm::Internal::ImageHeader {
    char magicWord[8];
    u32  endianessMark;
    u32  versionMark;
    u32  nTokens;
    u32  nNodes;          // excluding sentinel
    u32  nWordScores;     // excluding sentinel
    u32  tokensOffset;
    u32  nodesOffset;
    u32  scoresOffset;
    u32  end;
...
const char *BackingOffLm::Internal::ImageHeader::magic   = "MB020205";

/**
 * Image file format version number.
 ...
 */
const u32   BackingOffLm::Internal::ImageHeader::version = 1;
...
bool BackingOffLm::Internal::writeImage(int fd, const std::string &info) const {
    ...
    ImageHeader header;
    memcpy(header.magicWord, ImageHeader::magic, 8);
    header.endianessMark = 0x11223344;
    header.versionMark   = ImageHeader::version;
    header.tokensOffset  = 0; // phony
    header.nTokens       = tokens_.size();
    header.nodesOffset   = 0; // phony
    header.nNodes        = nNodes();
    header.scoresOffset  = 0; // phony
    header.nWordScores   = nWordScores();
    header.end           = 0; // phony

    // write phony header
    nBytes = sizeof(ImageHeader);
    if (write(fd, &header, nBytes) != nBytes) return false;

    // write info
    nBytes = info.size() + 1;
    if (write(fd, info.c_str(), nBytes) != nBytes) return false;

    // write token strings
    if ((position = lseek(fd, 0, SEEK_CUR)) == (off_t) -1) return false;
    header.tokensOffset = position;
    if (!writeImageTokenTable(fd)) return false;

    // write nodes
    if ((position = lseek(fd, 0, SEEK_CUR)) == (off_t) -1) return false;
    pad = (position + 7) % 8;
    if ((position = lseek(fd, pad, SEEK_CUR)) == (off_t) -1) return false;
    header.nodesOffset = position;
    nBytes = (nodesTail_ - nodes_ + 1) * sizeof(Node);
    if (write(fd, nodes_, nBytes) != nBytes) return false;

    // write word scores
    if ((position = lseek(fd, 0, SEEK_CUR)) == (off_t) -1) return false;
    pad = (position + 7) % 8;
    if ((position = lseek(fd, pad, SEEK_CUR)) == (off_t) -1) return false;
    header.scoresOffset = position;
    nBytes = (wordScoresTail_ - wordScores_ + 1) * sizeof(WordScore);
    if (write(fd, wordScores_, nBytes) != nBytes) return false;
    ...
    // write header
    if ((position = lseek(fd, 0, SEEK_SET)) == (off_t) -1) return false;
    nBytes = sizeof(ImageHeader);
    if (write(fd, &header, nBytes) != nBytes) return false;
    ...
    const Bliss::TokenInventory &inventory)
{
    ...
    if (memcmp(header.magicWord, ImageHeader::magic, 8)) {
	info = "bad magic word in image header";
	return false;
    }
    if (header.endianessMark != 0x11223344) {
	info = "image has incompatible byte order";
	return false;
    }
    if (header.versionMark != ImageHeader::version) {
	info = Core::form("image has version %d which is incompatible with currect version %d",
			  header.versionMark, ImageHeader::version);
	return false;
    }
    if (header.tokensOffset == 0) {
	info = Core::form("image seems to be incomplete");
	return false;
    }
    ...
    info = std::string(mmap_ + sizeof(ImageHeader));
